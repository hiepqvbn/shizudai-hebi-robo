...
def main():
    ...
    if argvs[1] == 'train':
        from modules.model_training.model import Model, save_model
        if len(argvs) >= 3:
            csv_file = argvs[2]
            try:
                ...
                iteration = 10
            visualize = argvs[3] == 'visualize' or argvs[4] == 'visualize' if len(
                argvs) >= 4 else False
            model = Model(C_space_arm_pos=np.zeros(3))
            model.train(csv_file=csv_file, iteration=iteration,
                        show_plot=visualize)
            save_model()
        else:
    ...
    if argvs[1] == 'sim':
        ...
        if len(argvs) >= 4:
            ...
            mode = None
        simulation_main(visualize, mode)
        ...
        class mainGUI(pygame_GUI.pygameGUI):
            ...
            def on_button_pressed(self, button):
                ...
                if button.name == 'button_b':
                    ...
                    collector.write_data_to_csv(self.robot.group_fbk.position)

        # detect_blue_mark.main()
        pyGUI = mainGUI()
        collector = data_collection.ExperimentDataCollector(cols=pyGUI.robot.names)

        # camera_thread = threading.Thread(target=detect_blue_mark.main, args=())
        # camera_thread.start()
        # rc = realsense_depth.DepthCamera()
        csvfile = pathlib.Path().absolute()/collector.csv_filename
