...
from modules.model_training.algorithm.grid_learning import *
...
class DataVisual():
    def __init__(self, csvfile=None, end_effector=None, k=1) -> None:
        ...
        if csvfile:
            ...
            # .assign(Index=range(len(self.df))).set_index('Index')
            self.C_points = self.df[['J1_base', 'J2_shoulder', 'J3_elbow']]
            ...
            self.grids = []
            for i in range(k):
                data = self.C_points[self.C_points['Kmeans_label']
                                     == i].iloc[:, 0:3]
                # print(data.head(3))
        ...
        if end_effector is not None:  # end_effector should be a numpy array
            ...
            self.ee_plot, = self.ax.plot(
                self.end_effector[0], self.end_effector[1], self.end_effector[2], 'ys')
    ...
    def scatter_plot3D(self, data, draw_samples=True, draw_grids=False, k=1):
        ...
        if draw_samples:
            self.ax.scatter(
                x, y, z, s=0.2, c=self.kmeans.labels_, cmap='rainbow')
